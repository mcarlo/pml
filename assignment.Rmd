...
### Model 2: _rfTrain2_
I also trained four random forest models - _rfBelt_, _rfArm_, _rfDumbbells_, _rfForearm_ - on 80% of the training set, using the default parameters from the caret package. According to their names, I confined each model to variables from its appropriate device location.

Using only the most important variables identified from each location - defining "important" as greater than 50 - I trained a random forest model on the same training data subset, using the default parameters from the caret package.

## 4. Results
### Important variables
Among the four location-specific random forest models, the following variables met the threshold of variable importance greater than 50.
...
```
## 5. Most important locations: belt, dumbbells, forearm
...
For any graders who insist on additional cross-validation, I also tested both models on the held-out training data. 

The first model's error prediction (based on the OOB error rate) is 0.57%. When tested on the cross-validation sample, the error rate was 0.7643%.
