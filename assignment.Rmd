...
Both approaches resulted in accuracy that rounds to 1.0. The expected out of sample error rates were 0.57% and 0.50%, respectively.
...
To eliminate variables not derived from wearable device measurements, I subset the training data to include only variables that reflected discrete measures from the wearable devices, leaving 52 possible predictor variables. These possible predictor variables comprised 13 variables for each of the four devices.

Then I subset the training data into fifths, randomly sampling the outcome variable, "classe."
...
## 3. Modeling
### Model 1: _rfTrain_
I trained a random forest model on 80% of the training set using the default parameters from the caret package.

### Model 2: _rfTrain2_
I also trained four random forest models - _rfBelt_, _rfArm_, _rfDumbbells_, _rfForearm_ - on 80% of the training set, using the default parameters from the caret package. I confined each model to a specific device location, hence their names.

Combining the most important variables from each location - defining "important" as greater than 50 - I trained a random forest model on the same 80% subset of the training data set used for _rfTrain_ using only the important variables and using the default parameters from the caret package.
...
### rfTrain results
_rfTrain_ is the model generated in a single step, training a random forest model on the entire training data set among all 52 predictor variables. Its predicted error rate, estimated from the OOB error rate used to select the best tree, was 0.57%.
...
```
## 5. Most important variable locations: belt, dumbbells, forearm
It is not surprising that measures from the arm location was not among the most important variables in either final model. I do find it surprising that the belt location was so important, which makes me wonder exactly how classes B, C, D, and E were performed that belt measurements were helpful at all. 
...
For graders who insist on cross-validation, I tested both models on the held-out training data. The first model's error prediction (based on the OOB error rate) is 0.57%. When tested on the cross-validation sample, the error rate was 0.7643%.

The second model's error prediction (based on the OOB error rate) is 0.50%. When tested on the cross-validation sample, the error rate was 0.6115%.

For either model, the expected likelihood of predicting all 20 test cases correctly on using _rfTrain2_ is approximately 0.995^20, or 0.90. 
...
cMTrain
trainError <- (1 - sum(diag(cMTrain$table))/sum(apply(cMTrain$table, 2, sum)))
trainError

cMTrain2 <- confusionMatrix(predTrain2, cvTrain$classe)
cMTrain2

train2Error <- (1 - sum(diag(cMTrain2$table))/sum(apply(cMTrain2$table, 2, sum)))
train2Error
```
