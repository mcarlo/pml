...
I compared two approaches, both using the random forest method. The first approach simply trained a random forest model on the entire training set. Unlike many other machine learning methods, random forest entails cross validation when building forests so there is no need for a modeler to hold out data for cross validation. (See random forest's documentation on this point at <http://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm#ooberr>)
...
```{r rfTest2,cache=TRUE,echo=FALSE,dependson=c('rsubset','rfMod','rftest')}
...
```{r plot1,cache=TRUE,echo=FALSE,dependson=c('rsubset','rfMod','rftest', 'rfTest2'),fig.align='center',fig.width=10,fig.height=8}
library(caret)
rfTrain$finalModel
...
```{r plot2,cache=TRUE,echo=FALSE,dependson=c('rsubset','rfMod','rftest', 'rfTest2'),fig.align='center',fig.width=10,fig.height=8}
library(caret)
rfTrain2$finalModel
...
```{r plot3,cache=TRUE,echo=FALSE,dependson=c('rsubset','rfMod','rftest', 'rfTest2'),fig.align='center',fig.width=10,fig.height=8}
library(caret)
