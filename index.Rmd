...
### rfTrain results
_rfTrain_ is the model I generated in a single step, training a random forest model on the entire training data set among all 52 predictor variables. Its predicted error rate, estimated from the OOB error rate used to select the best tree, was 0.57%.
...
### rfTrain2 results
_rfTrain2_ is the model I generated in two steps, with the first step training four random forest models on 20% of the training data, with each model confined to 13 predictor variables from one device location. The second step consisted of training a single random forest model on the entire training set using only variables whose importance was greater than 50 in the first step. Its predicted error rate, estimated from the OOB error rate used to select the best tree, was 0.50%.
